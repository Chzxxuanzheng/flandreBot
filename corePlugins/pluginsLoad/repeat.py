...
from nonebot.plugin.manager import Plugin, importlib, escape_tag
...
class PluginLoadRepeat:
	...
	def record(self, name: str, reason: Exception):
...
nonebot.plugin.load.PluginManager.load_plugin = loadMaker(__PluginStatus)

class DisablePlugin(Exception):
	def __init__(self, resaon: str = '未知原因'):
		super().__init__(resaon)

def getPluginLoadReport()-> PluginLoadRepeat:
	return __reapeat
